								...
								trigger_levels trigger_level)
{
	...
	if(this->trigger_level == trigger_levels::trigger_low)
		this->init((exti_line_enum)pin,gpio_periph,4,0,EXTI_INTERRUPT,EXTI_TRIG_FALLING,GPIO_PUPD_PULLUP);//上拉下降沿中断
	else
		this->init((exti_line_enum)pin,gpio_periph,4,0,EXTI_INTERRUPT,EXTI_TRIG_RISING,GPIO_PUPD_PULLDOWN);//下拉上升沿中断
...
static void button_callback(void *str)
{
	...
	if(((button_drv*)str)->Button_State==false)//按下触发
	{
		...
		if(((button_drv*)str)->trigger_level == button_drv::trigger_levels::trigger_low)
			...
			((button_drv*)str)->set_exti_trig(EXTI_TRIG_FALLING);//上升沿
	}else//松开触发
	{
		if(Deta_T<0.3f)
		{
